...
function buildPopupHtml(group) {
  ...
  const matchRows = sorted.map((m, i) => {
    ...
    const separator = i < sorted.length - 1 ? '<hr style="border:none;border-top:1px solid #333;margin:8px 0;">' : '';
    return `
      <div>
        <div style="color:#aaa;font-size:0.85em;">${formatDate(m.date)} · ${m.competition} · ${m.round} · ${m.leg}</div>
        <div style="margin-top:2px;">${m.home_team} – ${m.away_team}  <strong>${m.home_score}–${m.away_score}</strong></div>
      </div>${separator}`;
  }).join('');
  ...
  return `
    <div style="font-family:sans-serif;min-width:240px;">
      <div style="color:#007A33;font-weight:bold;font-size:1.1em;margin-bottom:6px;">📍 ${group.city}, ${group.country}</div>
      <hr style="border:none;border-top:1px solid #555;margin-bottom:8px;">
      ${matchRows}
      <hr style="border:none;border-top:1px solid #555;margin-top:8px;">
      <div style="color:#aaa;font-size:0.85em;margin-top:6px;">${total} meccs · ${wins} gy · ${draws} d · ${losses} v</div>
    </div>`;
}
